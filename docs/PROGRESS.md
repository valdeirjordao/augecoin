...
- CLI `send-transfer` (crates/augecoin-cli): constrói, assina (chave híbrida crua de arquivo — aceita formato `AUGECOIN_VALIDATOR_KEY_HEX=<hex>`, seed = primeiros 32 bytes) e envia Transaction via RPC. Busca `n_operation`/balance automaticamente (`getaccount`), valida fee mínima (MIN_FEE_AUGESAT) e saldo. Correção colateral: métodos JSON-RPC do cliente CLI estavam em camelCase contra servidor lowercase (`getaccount`, `sendoperation`, `nodestatus`, …) e campo `op_hex`→`hex` — comandos RPC do CLI voltaram a funcionar contra nós reais. Verificado e2e contra chain1 via `https://www.augeco.in/rpc` (rejeição esperada de assinatura com chave aleatória). 4 testes novos (parse de key file, roundtrip+verificação de assinatura, fee mínima, dependência da assinatura da chave); 46 no pacote. Nota: earnings/equivocation/banned/alerts seguem sem handler no servidor RPC (pré-existente).
- CLI `send-transfer --to-address`: destino por endereço AUGE via op `AddressTransaction` (0x0F). Endereços self-describing v2 (Base58 de pubkey Ed25519 + checksum `-PUBKEY`, ~49 chars) carregam a public key embutida — na execução, se o endereço não tem conta on-chain, o próprio nó ativa o primeiro AUGEID `Reserved` do SafeBox, vincula a pubkey ao destinatário (`state=Owned`) e credita o saldo no mesmo bloco; sem saldo pendente necessário. `--to` (conta) e `--to-address` são mutuamente exclusivos. 3 testes novos (destination obrigatória, roundtrip AddressTransaction com endereço embedded, endereço inválido); 49 no pacote.
